    ...
    if (post.pickup) {
      ...
      <div>
        ...
        <Snackbar
          ...
          autoHideDuration={1000}/>
  			<Card key={post.id}>
          <CardTitle title={course.department + ' ' + course.course_number + ': ' + post.title + ' (' + (post.condition) + ')'} subtitle={"$" + post.price + " | " + this.state.star_count + " Subscribers | " + moment(post.created_at).fromNow()}/>
          <CardHeader 
            title={"By " + seller}
            subtitle={mutual}
