class PostCreator
  ...
  def allowed_params(params, current_user)
    ...
    {price: params[:price], title: params[:title].capitalize, pickup: params[:pickup].capitalize, course_id: course_id, seller_id: current_user.id, picture_url: pic_url, description: params[:description].capitalize, condition: params[:condition]}
  end
end